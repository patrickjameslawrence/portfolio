...
import avatarImage from '@/assets/logos/portfolio.svg'
...
function ThemeToggle() {
  ...
  return (
    <button
      ...
      className="group rounded-md bg-white/90 px-3 py-2 shadow-lg ring-1 shadow-zinc-800/5 ring-zinc-900/5 backdrop-blur-sm transition dark:bg-zinc-800/90 dark:ring-white/10 dark:hover:ring-white/20"
    ...
    >
      ...
      <MoonIcon className="hidden h-6 w-6 fill-zinc-700 stroke-zinc-500 transition dark:block [@media_not_(prefers-color-scheme:dark)]:fill-cyan-400/10 [@media_not_(prefers-color-scheme:dark)]:stroke-cyan-500 [@media(prefers-color-scheme:dark)]:group-hover:stroke-zinc-400" />
    </button>
  )
}
...
function Avatar({
  ...
  return (
    <Link
      href="/"
      aria-label="Home"
      className={joinClassNames(className ?? '', 'pointer-events-auto')}
      {...props}
    >
      <Image
        src={avatarImage}
